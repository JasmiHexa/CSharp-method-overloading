...
namespace CSharpSyntheticRepo.Infrastructure
{
    public sealed class ExternalShippingClient
    {
        ...
        public ExternalShippingClient(ConsoleLogger logger)
        {
            _logger = logger;
        }
        ...
        {
            return CreateLabel(order.Id, order.Items.Sum(i => i.Quantity), order.CustomerEmail);
        }
        ...
        public Result<string> CreateLabel(string orderId, int itemCount, string destinationEmail)
        {
            ...
            var tracking = "TRK-" + orderId + "-" + itemCount.ToString("D2");
