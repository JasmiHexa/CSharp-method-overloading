...
namespace CSharpSyntheticRepo.Workflow
{
    public sealed class AppWorkflow
    {
        ...
        public Result Run()
        {
            var order = new Order
            {
                Id = "ORD-" + DateTime.UtcNow.ToString("yyyyMMdd-HHmmss"),
            ...
            var centsSubtotal = new Services.PricingService(_logger).CalculateSubtotal(7999, 1);
            _logger.Log(LogLevel.Debug, "Cents subtotal: " + centsSubtotal.ToString("0.00") + " USD");

            // Same method name, different params (PlaceOrder(order) vs PlaceOrder(order, couponCode))
            var tracking = _controller.PlaceOrder(order, "SAVE10");
            ...
            {
                Id = "ORD-" + DateTime.UtcNow.ToString("yyyyMMdd-HHmmss") + "-B",
